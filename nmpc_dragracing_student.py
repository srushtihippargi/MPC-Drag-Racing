...
def nmpc_controller(kappa_table = None):
    ...
    for k in range(N):
        ...
        cons_ineq.append(c1f1)##  front tire limits)
        cons_ineq.append(c1r2)## : reat  tire limits)        
    ...
    J = 0.0
    J = J + lane*100 + x[5,-1]**2*50 + 50*velo + 50*speed ##  terminal cost
    ...
    for k in range(N):
        ...
        speed = (x[3,k]-5000)**2
        J = J + lane*500 + x[5,k]**2*5 + 5*velo + 5*speed ## running cost
    ...
    ctrl_ub  = np.array([ 1e8,   0.4712]) # traction force | steering angle, use param["delta_max"]
